...
s_a[75]="Alsace|Amiens|Aquitaine|Auvergne|Basse-Normandie|Beauvais|Bourgogne|Bretagne|Centre|Champagne-Ardenne|Corse|Franche-Comte|Haute-Normandie|Ile-de-France|Languedoc-Roussillon|Limousin|Lorraine|Midi-Pyrenees|Nord-Pas-de-Calais|Pays de la Loire|Picardie|Poitou-Charentes|Provence-Alpes-Cote d'Azur|Rhone-Alpes";
...
s_a[112]="Abruzzo|Basilicata|Calabria|Campania|Emilia-Romagna|Empoli|Ferrara|Friuli-Venezia Giulia|Lazio|Liguria|Lombardia|Marche|Marghera|Molise|Padova|Piemonte|Prato|Puglia|Sardegna|Sicilia|Toscana|Trentino-Alto Adige|Umbria|Valle d'Aosta|Veneto|Vicenza";
...
var e_status = new Array("Student", "Full-time Employed", "Part-Time Employed", "Self-Employed", "Unemployed", "Entrepenuer");

var a_income = new Array("Less Than ₦1,200,000", "₦1,200,000 - ₦5,500,000", "₦5,500,000 - ₦10,000,000", "₦10,000,000 - ₦25,000,000", "₦25,000,000 - ₦40,000,000", "₦40,000,000 and above");

var s_income = new Array("Bonus | Gift", "Business", "Commission", "Salary", "Investment", "Rental", "Royalty");

var id_type = new Array("Drivers License", "National ID Card", "Permanent Voters Card", "International Passport");
...
function populateGender(genderElementId){
	// given the id of the <select> tag as function argument, it inserts <option> tags
	var genderElement = document.getElementById(genderElementId);
	genderElement.length=0;
	genderElement.options[0] = new Option('Select Gender','');
	genderElement.selectedIndex = 0;
	for (var i=0; i<gender.length; i++) {
		genderElement.options[genderElement.length] = new Option(gender[i],gender[i]);
	}
}

function populatee_Status(e_statusElementId){
	// given the id of the <select> tag as function argument, it inserts <option> tags
	var e_statusElement = document.getElementById(e_statusElementId);
	e_statusElement.length=0;
	e_statusElement.options[0] = new Option('Select Employment Status','');
	e_statusElement.selectedIndex = 0;
	for (var i=0; i<e_status.length; i++) {
		e_statusElement.options[e_statusElement.length] = new Option(e_status[i],e_status[i]);
	}
}

function populatea_Income(a_incomeElementId){
	// given the id of the <select> tag as function argument, it inserts <option> tags
	var a_incomeElement = document.getElementById(a_incomeElementId);
	a_incomeElement.length=0;
	a_incomeElement.options[0] = new Option('Select Annual Income','');
	a_incomeElement.selectedIndex = 0;
	for (var i=0; i<a_income.length; i++) {
		a_incomeElement.options[a_incomeElement.length] = new Option(a_income[i],a_income[i]);
	}
}

function populates_Income(s_incomeElementId){
	// given the id of the <select> tag as function argument, it inserts <option> tags
	var s_incomeElement = document.getElementById(s_incomeElementId);
	s_incomeElement.length=0;
	s_incomeElement.options[0] = new Option('Select Source of Income','');
	s_incomeElement.selectedIndex = 0;
	for (var i=0; i<s_income.length; i++) {
		s_incomeElement.options[s_incomeElement.length] = new Option(s_income[i],s_income[i]);
	}
}

function populateid_Type(id_typeElementId){
	// given the id of the <select> tag as function argument, it inserts <option> tags
	var id_typeElement = document.getElementById(id_typeElementId);
	id_typeElement.length=0;
	id_typeElement.options[0] = new Option('Select ID Type','');
	id_typeElement.selectedIndex = 0;
	for (var i=0; i<id_type.length; i++) {
		id_typeElement.options[id_typeElement.length] = new Option(id_type[i],id_type[i]);
	}
}
